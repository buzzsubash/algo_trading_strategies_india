...
first_entry_modified = 'no'
second_entry_modified = 'no'
...
def modify_order(order_id, price):  # SL-M Order Modification
    kite.modify_order(order_id=order_id, trigger_price=price, variety=kite.VARIETY_REGULAR)
...
if ce_sl_status == 'executed' and pe_sl_status == 'executed':
    calculate_atm_and_place_order()
    time.sleep(10)
print(dt.datetime.now().time() < sqf_time)
while dt.datetime.now().time() < sqf_time:
    if second_entry_modified == 'no':
        ce_sl_status = get_order_status(ce_sl_orderid)
        pe_sl_status = get_order_status(pe_sl_orderid)

        if ce_sl_status == 'executed':
            modify_order(pe_sl_orderid, pe_sell_price)
            second_entry_modified = 'yes'
        elif pe_sl_status == 'executed':
            modify_order(ce_sl_orderid, ce_sell_price)
            second_entry_modified = 'yes'
    time.sleep(5)
