...
def get_expiry_date():
    """
    Calculates the expiry date of the current trading week, assuming expiry is on Thursday.
    Adjusts for holidays and considers today as the expiry date if it's Friday and is the expiry day.
    ...
    if wd == 4:  # Friday
        # Assuming we have a way to check if today is the expiry day, for simplification let's just consider every Friday as a potential expiry day
        # In a real scenario, you might check against a list of expiry dates or some other condition
        is_expiry_today = True  # This should be determined by checking if today's date is in a list of expiry dates or some other logic
...
def get_nifty_ltp():
    """
    Retrieves the Last Traded Price (LTP) for the SENSEX index from the Kite Connect API.
    ...
    Returns:
        float: The last traded price of the SENSEX index.
...
def get_nifty_atm_strike(nifty_ltp):
    """
    Calculates the nearest At The Money (ATM) strike price for Sensex options based on the Last Traded Price (LTP) of Sensex.

    The Sensex options are available in strike price intervals of 50. This function rounds the LTP to the nearest
    ...
    Parameters:
        nifty_ltp (float): The last traded price of Sensex.
...
def get_trading_symbol(df, strike, CE_or_PE):
    """
    Retrieves the trading symbol for a specific Sensex option based on the strike price and option type (CE for Calls, PE for Puts).
...
def get_ce_and_pe_ltp(ce_symbol, pe_symbol):
    ...
    This function constructs the full trading symbols for the options by prefixing them with 'BFO:'
    to denote the Sensex Futures and Options segment. It then attempts to fetch the LTPs for these
    ...
    # Prefix the given symbols with 'BFO:' to specify the Sensex Futures and Options segment.
    ce = 'BFO:' + ce_symbol
    pe = 'BFO:' + pe_symbol
...
def marketorder_buy(symbol, quantity):
    ...
    The order is placed in the Sensex Futures and Options segment (BFO) as a normal (NRML) product, which implies
    ...
    return kite.place_order(
        ...
        exchange=kite.EXCHANGE_BFO,  # The exchange segment (BFO for Sensex Futures and Options).
...
def marketorder_sell(symbol, quantity):
    ...
    The order is placed in the Sensex Futures and Options segment (BFO) as a normal (NRML) product. This means the order
    ...
    return kite.place_order(
        ...
        exchange=kite.EXCHANGE_BFO,  # The exchange segment for the order (BFO for Sensex Futures and Options).
...
# Try to download the instrument dump from the BFO segment up to 16 times (0 through 15).
a = 0
while a <= 15:
    try:
...
# Filter the DataFrame to include only instruments with the name 'Sensex'.
nifty = instrument_df[instrument_df.name == 'SENSEX']

# Calculate the expiry date for the current trading cycle.
expiry_date = get_expiry_date()

# Further filter the DataFrame to include only Sensex instruments with the calculated expiry date.
...
# Fetch the current Last Traded Price (LTP) of Sensex.
nt_ltp = get_nifty_ltp()
print('SENSEX : ', nt_ltp)

# Calculate the At The Money (ATM) strike price based on the Sensex LTP.
...
if ce_status == 'COMPLETE' and pe_status == 'COMPLETE':
    ...
    while True:
        ...
        print('MTM ', round((sell_premium - current_premium) * 10 * lots), 2)
