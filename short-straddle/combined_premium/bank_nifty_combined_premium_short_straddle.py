...
def get_expiry_date():
    ...
    if wd < 3:
        exp_date = current_date + dt.timedelta(days=2 - wd)  # Adjust for Wednesday
    elif wd == 3:
        exp_date = current_date - dt.timedelta(days=1)  # If today is Thursday, go back to Wednesday
    else:
        exp_date = current_date + dt.timedelta(days=9 - wd)  # Adjust for next Wednesday
...
if ce_status == 'COMPLETE' and pe_status == 'COMPLETE':
    ...
    while True:
        ...
        print('MTM ', round((sell_premium - current_premium) * 15 * lots), 2)
